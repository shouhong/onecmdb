...
:: MAIN Start point
::CALL "%ONECMDB_HOME%\bin\generate-transform.bat" --url %ONECMDB_URL%/webservice/onecmdb --input conf/nmap2cmdb.xml --type xml --output conf/nmap2cmdb-transform.xml
::IF %ERRORLEVEL% NEQ 0 GOTO transformerror

ECHO Start transform...
CALL "%ONECMDB_HOME%/bin/transform.bat"  --valueMap {network}=%NAME% --token %TOKEN% --transform conf/nmap2cmdb.xml  --transformType simple --fileSource "%ONECMDB_DESKTOP_URL%/onecmdb/export?name=NMAP/nmap.cfg" --sourceType xml  --name nmap2cmdb --output result/nmap2cmdb_result.xml
IF %ERRORLEVEL% NEQ 0 GOTO transformerror

:: If history entry is specified upload result to changehandler.
IF [%HISTORY%] == [] goto end
ECHO Upload result to change manager.
CALL "%ONECMDB_HOME%/bin/upload.bat" --token %TOKEN% --postURL %ONECMDB_DESKTOP_URL%/onecmdb/change --input result\nmap2cmdb_result.xml --history %HISTORY%
