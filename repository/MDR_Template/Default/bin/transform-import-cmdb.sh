...
echo "Options:" 
echo "--name name - A name for this transform" 
...
# Main 
"$ONECMDB_HOME/bin/transform.sh" --token $TOKEN --transform conf/$NAME/transform.xml --transformType simple --source conf/$NAME/source.xml  --output result/result_$NAME.xml 
if [ "$?" != "0" ] 
then 
